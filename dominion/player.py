...
class WebsocketPlayer(PlayerHandle):
    def __init__(self, name, websocket):
        ...
        self.websocket: SocketIO = websocket
    ...
    def notify_player_joined(self, player):
        self.websocket.emit('message', self._make_msg(locals()))
        eventlet.sleep()
    ...
    def notify_started_action_phase(self, player):
        ...
        if player == self.name:
            ...
            while breaker:
                ...
                self.websocket.on_event('finish_action_phase', finish_action_phase)
                eventlet.sleep()
                self.websocket.on_event('play_card', play_card)
                eventlet.sleep()
    ...
    def notify_started_buy_phase(self, player):
        ...
        if player == self.name:
            ...
            while breaker:
                ...
                self.websocket.on_event('finish_turn', finish_turn)
                eventlet.sleep()
                self.websocket.on_event('buy_card', card_buy)
                eventlet.sleep()
                # message = json.loads(self.websocket.receive())
    ...
    def choose_card_from(self, collection):
        ...
        self.websocket.emit('message', msg)
        eventlet.sleep()

        while True:

            # card = json.loads(self.websocket.receive())
            card = ''
            ...
            self.websocket.on_event('chosen_card', get_card)
            eventlet.sleep()
    ...
    def ask_yes_or_no(self, prompt):
        ...
        while True:
            ...
            def get_choice(message):
                nonlocal answer
                answer = message
                nonlocal websocket_received
                websocket_received = True

            self.websocket.on_event('answer_yes_or_no', get_choice)
            eventlet.sleep()
            # response = json.loads(self.websocket.receive())

            if websocket_received:
                return answer
